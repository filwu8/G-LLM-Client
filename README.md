...
- 更新 XML/YAML 传递依赖至修复版本，解决发布审计发现的高危安全公告。
...
- 新增自主目标模式：设定最终目标、验收标准、授权文件夹、执行权限、最大轮数和单次运行时长后，助手可持续规划、调用工具、修改文件并验证结果。
- 目标状态和执行计划会随会话保存在本机；达到时间或轮数上限、客户端重启或用户暂停后，可检查现有成果并继续执行。
- 参考 Codex 的交互逻辑，明确区分暂停、继续与取消目标；取消会立即停止运行并移除目标，但保留聊天记录、生成文件和工作区授权。
- 主窗口与小窗口同步显示紧凑的当前目标、绿色步骤状态、持续运行提示、实时生成文件，以及可快捷打开的工作区文件夹。
- 普通停止按钮在目标运行期间会正确将目标切换为暂停，避免执行已停止但界面仍显示运行中的状态不一致。
- 模型管理新增搜索框，可按模型名称或标识快速过滤长列表。
