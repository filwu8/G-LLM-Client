...
export function selectWorkspaceEnvironment(values: Record<string, string>, names: string[]): Record<string, string> {
  ...
  for (const name of normalizeEnvNames(names)) {
    ...
    if (/^(?:PATH|HOME|SHELL|ENV|BASH_ENV|IFS|CDPATH|NODE_OPTIONS|NODE_PATH|ELECTRON_.*|PYTHON.*|LD_.*|DYLD_.*|COMSPEC|SYSTEMROOT|WINDIR|SYSTEMDRIVE|LOCALAPPDATA|PSMODULEPATH|PSMODULEANALYSISCACHEPATH|TEMP|TMP|TMPDIR)$/i.test(name)) {
