...
登录 new-api 后自动配置 API Key 时，网页必须先调用 `POST /api/client/handoff` 获取一次性授权码，再打开服务端返回的深链接：

```text
gllm://open?source=new-api&handoff=<64 位一次性授权码>
```
...
- host：`open`
- path：空路径或根路径 `/`
- 参数：无参数、唯一参数 `source=new-api`，或严格顺序的 `source=new-api&handoff=<64 位字母数字授权码>`
